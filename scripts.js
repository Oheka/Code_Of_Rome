...
const el = {
    ...
    worksheet:    document.getElementById('cipherWorksheet'),
    worksheetGrid:document.getElementById('worksheetGrid'),
};
...
function buildWorksheet(cipherText) {
    // Find which letters actually appear in ciphertext
    const usedLetters = new Set(cipherText.toUpperCase().replace(/[^A-Z]/g, ''));

    el.worksheetGrid.innerHTML = '';
    for (const letter of ALPHABET) {
        const cell = document.createElement('div');
        cell.className = 'ws-cell' + (usedLetters.has(letter) ? ' active' : '');
        cell.id = `ws-${letter}`;

        const top = document.createElement('div');
        top.className = 'ws-cipher';
        ...
        inp.setAttribute('aria-label', `Plain letter for cipher ${letter}`);

        inp.addEventListener('input', e => {
            const v = e.target.value.replace(/[^a-zA-Z]/g, '').slice(-1).toUpperCase();
            inp.value = v;
            cell.classList.toggle('filled', v.length > 0);
            // Auto-advance to next active cell
            if (v) {
                const cells = [...el.worksheetGrid.querySelectorAll('.ws-cell.active .ws-input')];
        ...
        el.worksheetGrid.appendChild(cell);
    }
    el.worksheet.style.display = 'block';
}

function clearWorksheet() {
    el.worksheetGrid.querySelectorAll('.ws-input').forEach(inp => {
        inp.value = '';
        inp.closest('.ws-cell').classList.remove('filled');
    });
}

function hideWorksheet() {
    el.worksheet.style.display = 'none';
    el.worksheetGrid.innerHTML = '';
}
...
function startGame() {
    ...
    resetHint();
    buildWorksheet(caesarCipher(currentPhrase, currentShift));
...
function playAgain() {
    ...
    el.checkButton.style.display = 'none';
    el.feedback.style.display = 'none';
    resetHint(); updateProgress(); updateStats(); closeVictoryModal(); hideWorksheet(); scrollToGame();
}
